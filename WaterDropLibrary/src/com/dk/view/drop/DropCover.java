...
public class DropCover extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void finish(View target, float x, float y) {
        double distance = Math.sqrt(Math.pow(mBaseX - mTargetX, 2) + Math.pow(mBaseY - mTargetY, 2));
       float tempx=(float) ((mBaseX-mTargetX)*5/distance);
       float tempy=(float) ((mBaseY-mTargetY)*5/distance);
        clearDatas();
        Canvas canvas = getHolder().lockCanvas();
        if (canvas != null) {
            canvas.drawColor(Color.TRANSPARENT, Mode.CLEAR);
        ...
        if (distance > mMaxDistance) {
            ...
            mThread.start();
             ...
             TranslateAnimation anima=new TranslateAnimation(tempx, -tempx, tempy, -tempy);
             anima.setDuration(100);
             anima.setRepeatCount(5);
             anima.setRepeatMode(Animation.REVERSE);
             anima.setInterpolator(new OvershootInterpolator());
             target.startAnimation(anima);
        }
